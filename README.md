...
- `off` — No OpenCode output; agent-sync-managed `opencode.json` (stamp-gated) and `.opencode/` are reconciled away / 不生成 OpenCode 输出；由 agent-sync 管理的 `opencode.json`（stamp-gated）和 `.opencode/` 会被回收
- `native` (default) — All three outputs above / 三种输出全部生成

**Stamp-gated ownership / Stamp 守门**: The generated `opencode.json` stays within OpenCode's strict schema; ownership is tracked by `.opencode/.config-json-agent-sync`. Any hand-authored `opencode.json` without this stamp is **never** written to, cleaned, or reconciled — it is treated as user-owned. Pre-HIST-009 configs that still contain `"_generated_by": "agent-sync"` are treated as managed once, then rewritten without the legacy key. / 生成的 `opencode.json` 保持 OpenCode strict schema；所有权由 `.opencode/.config-json-agent-sync` 记录。没有该 stamp 的手写 `opencode.json` **不会**被 sync / clean / reconcile 触碰。仍含 `"_generated_by": "agent-sync"` 的 pre-HIST-009 配置会被视为旧版管理文件，并在下一次 sync 中迁移为无 legacy key 的配置。
...
| OpenCode config | `opencode.json` is valid JSON; agent-sync-managed files carry `.opencode/.config-json-agent-sync` and reference `.cursor/rules/*.mdc` (when OpenCode Mode = native) / `opencode.json` JSON 有效；若归 agent-sync 管理，则带 `.opencode/.config-json-agent-sync` stamp 并引用 `.cursor/rules/*.mdc` |
...
- **Auto-handled on the next `agent-sync` run** (zero action required) / **下一次 `agent-sync` 自动处理**（无需手动操作）
  - `opencode.json` at project root (stamp-gated) — references existing `.cursor/rules/*.mdc` / `.claude/rules/*.md` / `.agent-local.md`, no duplicate rule compilation. Verified by regression tests `T20` / `T22` / `T22b` / `T23`. / 根目录 `opencode.json`（stamp-gated），复用已有规则文件，不重复编译。由 `T20` / `T22` / `T22b` / `T23` 覆盖。
...
- **Opting out** / **关闭 OpenCode**:
  - Set `**OpenCode Mode**: off` in `.agent-local.md` and re-sync. `reconcile_mode_outputs()` removes stamp-gated `opencode.json`, sweeps the two OpenCode manifests, and `rmdir`s `.opencode/` when empty. User-owned `opencode.json` (no stamp) is preserved. Covered by `T21` / `T22`. / 在 overlay 加 `**OpenCode Mode**: off` 后重新 sync。`reconcile_mode_outputs()` 会移除带 stamp 的 `opencode.json`、回收两个 manifest，并在 `.opencode/` 空时 `rmdir`。用户手写的 `opencode.json` 不受影响。由 `T21` / `T22` 覆盖。

- **Coexisting with a hand-authored `opencode.json`** / **与手写 `opencode.json` 共存**:
  - The absence of `.opencode/.config-json-agent-sync` disables all `agent-sync` writes on that file (sync / clean / reconcile). To hand back ownership to `agent-sync`, delete the file and re-run. Covered by `T22`. / 缺失 `.opencode/.config-json-agent-sync` 时，`agent-sync` 不会碰现有 `opencode.json`。想把所有权交还给 `agent-sync`，删除该文件后重新 sync。由 `T22` 覆盖。
  - Legacy configs containing `"_generated_by": "agent-sync"` are auto-migrated to stamp ownership because OpenCode rejects unknown top-level keys. Covered by `T22b`. / 仍含 `"_generated_by": "agent-sync"` 的旧配置会自动迁移为 stamp ownership，因为 OpenCode 会拒绝未知顶层 key。由 `T22b` 覆盖。
...
- **Scope** / **影响面**:
  ...
  - A user-authored `opencode.json` (no stamp) in the repo root is also untouched. / 用户手写的根目录 `opencode.json` 不会被触碰。

- **History / 历史上下文**: see `issue_history/HISTORY.md` → `HIST-006: OpenCode 原生集成` and `HIST-009: OpenCode config ownership 改为 external stamp`. / 详见 `HIST-006` 与 `HIST-009`。
